...
If either $\phi$ or $\psi$ is true, then  $\omega$ will always be true.\\
\item
%\leavevmode\vadjust{\vspace{-\baselineskip}}\newline
\begin{tikzpicture}[baseline=(current  bounding  box.center), thick]
\node [circle, draw, fill=black, minimum size=.8cm] (a1) at (0,0) {};
...
\label{tab:Q2_7}
\end{table}
If $\chi$ is true at $i$ and $\chi$ \& $\psi$ is true at $i+1$, then $\omega$ can become an invariant anywhere on the path.\\
\newpage
\item
%\leavevmode\vadjust{\vspace{-\baselineskip}}\newline
\begin{tikzpicture}[baseline=(current  bounding  box.center), thick]
...
\node [circle, draw, fill=black, minimum size=.8cm] (a5) at (10,0) {};
\node [circle, draw, fill=black, minimum size=.8cm] (a6) at (12.5,0) {};
\node [circle, minimum size=.9cm] (7) at (15,0) {};\
...
If $\chi$ is true at $i$ and $\psi$ is true at $i+1$, then  $\omega$ will be true until $\omega$. If $\mu$ is true at $j$, then $\omega$ will be true at $j+5$.\\
\end{enumerate}
\end{document}